...
#関連付けリストの投稿日が空白の物を抽出
    ...
    relation  = relations.order("updated_at").limit(400)
...
if relation.count > 0
    relation.each do |data|
        ...
        #メディアがthisavの場合の投稿情報格納
    when "thisav"
    tomvdata = Thisavlist.where('thisavid = ?', data.mvid).first

    #アフィリリンクテーブルからランダムにURLを取得する
    afilink = Afilinklist.offset( rand(Afilinklist.count) ).first.afilinkurl
    #本文
    ##ヘッダー画像（サムネイルから取得）
    #thumbimage =%(<img src="http://img1.smv.to//<%= Youflixlist.thisavid %>/animation_00001.jpg" width="640" height="360" title="犬とサッカー">)
    thumbimage =%(https://static.thisav.com/images/videothumbs/#{tomvdata.thisavid}-1.jpg)
    post_content = %(<a href="#{afilink}"><img src="#{thumbimage.to_s}" width="640" height="360" alt="#{tomvdata.thisavtitle.to_s}"></a>)
    #H2タグ
    post_content <<  "<h2>「" +  tomvdata.thisavtitle + "」の無料動画情報</h2><br>"
    #説明文
    #post_content <<  "<p>" +  tomvdata.explanation.to_s + "</p><br>"

    post_content << %(<br>)
    #埋め込み動画
    post_content << %(<iframe scrolling="no" width="640px" height="360px" frameborder="0" src="http://www.thisav.com/video/embed/#{tomvdata.thisavid.to_s}/"></iframe><br>)

    #投稿元サイト
    post_content << %(投稿元サイト：<a href="https://www.thisav.com/video/#{tomvdata.thisavid.to_s}/">thisav(動画掲載ページ直リンク)</a>)

    #投稿名
    post_name = tomvdata.thisavid.to_s
    p post_name.to_s
    #投稿タイトル
    post_title = tomvdata.thisavtitle
    p post_title.to_s
    #タグデータ格納
    postgenrestr = postgenreid.to_s + "," + data.keyword.to_s + "," + tomvdata.tags.to_s
    #カテゴリデータ格納
    postcategorystr = postgenreid.to_s + ",thisav"
        ...
        #メディアが「Sharemovieの場合の投稿情報格納
            ...
            thumbimage =%(http://img1.smv.to/#{tomvdata.smvid}/thumbnail_13.jpg)
            post_content = %(<a href="#{afilink}"><img src="#{thumbimage.to_s}" width="640" height="360" alt="#{tomvdata.smvtitle.to_s}"></a>)
            #H2タグ
            post_content <<  "<h2>「" +  tomvdata.smvtitle + "」の無料動画情報</h2><br>"
            #説明文
            post_content <<  "<p>" +  tomvdata.explanation.to_s + "</p><br>"
            #サムネイル
=begin     サムネイル画像が一定期間で利用できなくなるため、掲載を辞める 
...
=end        サムネイル画像が一定期間で利用できなくなるため、掲載を辞める 
            post_content << %(<br>)
            #埋め込み動画
            post_content << %(<iframe src="http://smv.to/embed/#{tomvdata.smvid.to_s}/" frameborder=0 width="640" height="360" scrolling=no></iframe><br>)
            #投稿元サイト
            post_content << %(投稿元サイト：<a href="http://smv.to/detail/#{tomvdata.smvid.to_s}">sharemovie(動画掲載ページ直リンク)</a>)
            #投稿名
        ...
        #メディアが「youflixの場合の投稿情報格納
            ...
            thumbimage =%(http://thumb1.youflix.is/#{tomvdata.youflixid}/thumbnail.jpg)
            post_content = %(<a href="#{afilink}"><img src="#{thumbimage.to_s}" width="640" height="360" alt="#{tomvdata.youflixtitle.to_s}"></a>)
            #H2タグ
...
=end        サムネイル画像が一定期間で利用できなくなるため、掲載を辞める 
            ...
            #投稿元サイト
            post_content << %(投稿元サイト：<a href="http://youflix.is/detail/#{tomvdata.youflixid.to_s}">Youflix(動画掲載ページ直リンク)</a>)
            #投稿名
        ...
        # テーブル名が命名規則に沿わない場合、
        self.table_name = 'wp_4_posts'  # set_table_nameは古いから注意
            self.primary_key = :ID
        end
        ...
        if user = User2.where("post_name = ?", post_name).exists?
            ...
            class User3 < ActiveRecord::Base
                # テーブル名が命名規則に沿わない場合、
                self.table_name = 'wp_4_postmeta' # set_table_nameは古いから注意
            ...
            # テーブル名が命名規則に沿わない場合、
                self.table_name = 'wp_4_term_taxonomy' # set_table_nameは古いから注意
            ...
            class Termstbl < ActiveRecord::Base
                # テーブル名が命名規則に沿わない場合、
                self.table_name = 'wp_4_terms' # set_table_nameは古いから注意
                ...
                if user = Termstbl.where("name = ?", genrestr.to_s.tr('０-９ａ-ｚＡ-Ｚ', '0-9a-zA-Z')).exists?
                    ...
                    if user2 = Termtaxonomy.where("term_id = ? and taxonomy = ?",user.term_id,'post_tag').exists?
                        ...
                        class Termsrelationtbl < ActiveRecord::Base
                            # テーブル名が命名規則に沿わない場合、
                            self.table_name = 'wp_4_term_relationships' # set_table_nameは古いから注意
