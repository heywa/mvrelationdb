...
=end        サムネイル画像が一定期間で利用できなくなるため、掲載を辞める 
        ...
        # テーブル名が命名規則に沿わない場合、
        self.table_name = 'wp_17_posts'  # set_table_nameは古いから注意
            self.primary_key = :ID
        end
        ...
        if user = User2.where("post_name = ?", post_name).exists?
            ...
            class User3 < ActiveRecord::Base
                # テーブル名が命名規則に沿わない場合、
                self.table_name = 'wp_17_postmeta' # set_table_nameは古いから注意
            ...
            # テーブル名が命名規則に沿わない場合、
                self.table_name = 'wp_17_term_taxonomy' # set_table_nameは古いから注意
            ...
            class Termstbl < ActiveRecord::Base
                # テーブル名が命名規則に沿わない場合、
                self.table_name = 'wp_17_terms' # set_table_nameは古いから注意
                ...
                if user = Termstbl.where("name = ?", genrestr.to_s.tr('０-９ａ-ｚＡ-Ｚ', '0-9a-zA-Z')).exists?
                    ...
                    if user2 = Termtaxonomy.where("term_id = ? and taxonomy = ?",user.term_id,'post_tag').exists?
                        ...
                        class Termsrelationtbl < ActiveRecord::Base
                            # テーブル名が命名規則に沿わない場合、
                            self.table_name = 'wp_17_term_relationships' # set_table_nameは古いから注意
