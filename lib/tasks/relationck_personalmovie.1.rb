...
#Thisav検索処理開始
mediasitename = "thisav"
if Thisavlist.where("thisavtitle like ? or  tags like ?", "%#{searchkeyword.to_s}%","%#{searchkeyword.to_s}%").exists?
#p "not include space"
fechcnt=Thisavlist.where("thisavtitle like ? or  tags like ?", "%#{searchkeyword.to_s}%","%#{searchkeyword.to_s}%").all.count
fechids=Thisavlist.where("thisavtitle like ? or  tags like ?", "%#{searchkeyword.to_s}%","%#{searchkeyword.to_s}%")
    fechids.each do |fechid|
    #無修正ものはなるべく回避する
    if fechid.thisavtitle.to_s.include?("無修正") || fechid.tags.to_s.include?("無修正")
      next
    end
    if fechid.thisavtitle.to_s.include?("（無）") || fechid.tags.to_s.include?("（無）")
      next
    end
    if fechid.thisavtitle.to_s.include?("(無)") || fechid.tags.to_s.include?("(無)")
      next
        ...
        if Genrerelationlist.where('mvid = ?', fechid.thisavid).where('genrerelationid = ?', genrerelationword).where('medianame = ?', mediasitename).exists?
        #すでにテーブルに登録済みの場合は何もしない。
        p fechid.thisavid.to_s + "is already exist skip!"
        updatecnt = updatecnt + 1
        else
        #テーブルに存在しない場合は情報をGenrerelationlistに挿入していく
        user = Genrerelationlist.new
        user.mvid = fechid.thisavid
        user.genrerelationid = genrerelationword
        user.medianame = mediasitename
        user.keyword = searchkeyword
        user.save
