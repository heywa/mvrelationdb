...
#関連付けリストの投稿日が空白の物を抽出
    ...
    relation  = relations.order("updated_at").limit(4)
    ...
    relation.each do |data|
            ...
            if tempstr == ""
               tempstr = str
                ...
                if actressstr == ""
                   ...
                   actressshtml = URI.escape(str + "(" + tempstr + ")")
                   postactresshtml =%(#{postactresshtml.to_s}<a href="http://unforgiven.tokyo/wordpress/freemovie/?s=#{actressshtml}">#{str}(#{tempstr})</a>)
                   tempstr =""
                else
                   actressstr = actressstr + "," + str + "(" + tempstr + ")"
                   actressshtml = URI.escape(str + "(" + tempstr + ")")
                   postserieshtml =%(#{postactresshtml.to_s}<p>,</p><a href="http://unforgiven.tokyo/wordpress/freemovie/?s=#{actressshtml}">#{str}(#{tempstr})</a>)
                   tempstr =""
                end
            end
            end   
        ...
        hoge.each do|str|
            ...
            if tempstr == ""
               ...
               direcrtorhtml = URI.escape(str + "(" + tempstr + ")")
               postdirecrtorhtml =%(#{postdirecrtorhtml.to_s}<a href="http://unforgiven.tokyo/wordpress/freemovie/?s=#{direcrtorhtml}">#{str}(#{tempstr})</a>)
               tempstr =""
            end
            end   
        end
        ...
        Relationlist.where('frommvid = ?', data.frommvid).each do |mvid|
           ...
           mvembe << %(<p>掲載元（ThisAV）のタイトル：<a href="htttps://www.thisav.com/video/embed/#{mvid.tomvid.to_s}/">#{thsismvdata.thisavtitle.to_s}</a></p><br>)
        ...
        #2018/05/12動画が古くなるとサムネイル画像がなくなるため、サムネイル画像は挿入しないように変更
=begin
        Relationlist.where('frommvid = ?', data.frommvid).each do |mvid|
            case mvid.relationid.to_s
                when "dmmtosmv" 
...
=end
        ...
        #2018/06/02DMMのDVD通販テーブルからタイトルで検索してメーカー品番をゲット
        p data.frommvid
        dmmdata = Dmmlist.where('dmmid = ?', data.frommvid).first
        if dvddata = Dmmdvdlist.where("dmmtitle = ?", dmmdata.dmmtitle).exists?
        dvddata = Dmmdvdlist.where("dmmtitle = ?", dmmdata.dmmtitle).order("makerid").first
        makerid = dvddata.makerid.to_s
        post_title =  "【" + makerid.to_s + "】" + post_title.to_s
        end
        #2018/06/02追記　ENDｰｰｰｰｰｰｰｰｰｰｰｰｰｰ
        
        post_content <<  "<h2>「" +  dmmdata.dmmtitle + "」の動画情報</h2><br>"
        post_content <<  "<p>DMM品番：" +  dmmdata.dmmid.to_s + "</p><br>"
        post_content <<  "<p>メーカー品番：" +  makerid.to_s + "</p><br>"
        post_content <<  "<p>配信日・発売日：" +  dmmdata.instoredate.to_s + "</p><br>"
        post_content <<  "<p>女優名：" +  postactresshtml.to_s + "</p><br>"
        postactresshtml = nil
        post_content <<  "<p>監督名：" +  postdirecrtorhtml.to_s + "</p><br>"
        postdirecrtorhtml = nil
        post_content <<  "<p>タグ・ジャンル：" +  postgenre.to_s + "</p><br>"
        #シリーズ名
        postseries = dmmdata.series
        serieshtml = %(URI.escape(dmmdata.series.to_s))
        postserieshtml =%(<a href="http://unforgiven.tokyo/wordpress/freemovie/?s=#{serieshtml}">#{postseries}</a>)
        post_content <<  "<p>シリーズ名：" +  postserieshtml + "</p><br>"
        #メーカー名
        postmaker = dmmdata.maker
        makerhtml = %(URI.escape(postmaker.to_s))
        postmakerhtml =%(<a href="http://unforgiven.tokyo/wordpress/freemovie/?s=#{makerhtml}">#{postmaker}</a>)
        post_content <<  "<p>メーカー名：" +  postmakerhtml + "</p><br>"
        #レーベル名
        postlabel = dmmdata.label
        labelhtml = %(URI.escape(postlabel.to_s))
        postlabelhtml =%(<a href="http://unforgiven.tokyo/wordpress/freemovie/?s=#{labelhtml}">#{postlabel}</a>)
        post_content <<  "<p>レーベル名：" +  postlabelhtml + "</p><br>"
        #サンプル画像
        if sampleimages==""
        else
...
=end            
        #2018/06/03　女優名、メーカー品番、シリーズ、監督名をタグに登録する
        #ジャンルを個別タグを挿入する
        # テーブルにアクセスするためのクラスを宣言
        ...
        #タグに登録する言葉をtagstblへカンマ区切りで格納
        tagstbl = %(#{postseries},#{makerid},#{postmaker},#{direcrtorstr},#{postactressstr})
        tagstrs = tagstbl.tr('０-９ａ-ｚＡ-Ｚ', '0-9a-zA-Z').split(",").uniq
        p 'tagstrs:' + tagstrs.to_s
        tagstrs.each do |tagstr|
        #タグの文字列が２００を超えてしまう場合、テーブル規則として格納できないため、スキップ
        if URI.escape(tagstr).length > 190
            next
        end
        #タグが存在するかチェックする。タグに全角英数字が混じっている場合、半角に統一する（2018/02/03）
        if user = Termstbl.where("name = ?", tagstr.to_s.tr('０-９ａ-ｚＡ-Ｚ', '0-9a-zA-Z')).exists?
        #タグがすでに存在する場合、カテゴリと重複していないか確認する
        user = Termstbl.where("name = ?", tagstr.to_s).first 
        if user2 = Termtaxonomy.where("term_id = ? and taxonomy = ?",user.term_id,'post_tag').exists?
            #タグがすでに存在する場合
            user = Termstbl.where("name = ?", tagstr.to_s).first 
            user.slug = URI.escape(tagstr)
            user.save
            # テーブルにアクセスするためのクラスを宣言
