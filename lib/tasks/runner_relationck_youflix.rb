...
=end
  ...
  #必要な変数定義
  #
  #Youflixlist.where('youflixid = ?', mvurl).exists?
  ...
  dmm_mvs.each do |dmm_mv|
    fechcnt=0
    fechids=""
    if Youflixlist.where("youflixtitle like '%" + dmm_mv.dmmtitle + "%'").exists?
    #p "not include space"
    fechcnt=Youflixlist.where("youflixtitle like '" + dmm_mv.dmmtitle + "'").all.count
    fechids=Youflixlist.where("youflixtitle like '" + dmm_mv.dmmtitle + "'")
    else
    #空白があるか
    ...
    #区切られた文字列を一つずつ取り出しand条件で検索
      if Youflixlist.where("youflixtitle like '%" + titlestr[0] + "%'").exists?
        #p titlestr[0]
      resultrec=Youflixlist.where("youflixtitle like '%" + titlestr[0] + "%'")
      titlestr.each do |str|
        #p str
        resultcnt=resultrec.where("youflixtitle like '" + str + "%'")
        if resultcnt.all.count == 0
          #p "separate str not match"
           ...
           resultrec=resultrec.where("youflixtitle like '%" + str + "%'")
           fechcnt=resultrec.where("youflixtitle like '%" + str + "%'").all.count
           fechids=resultrec
        end
        #p fechcnt.to_s
      end
    ...
    if fechcnt==0
       ...
       fechids.each do |fechid|
         ...
         if relation = Relationlist.where('frommvid = ?', dmm_mv.dmmid).where('tomvid = ?', fechid.youflixid).exists?
         #if Relationlist.where('frommvid = ?','tomvid = ?', dmm_mv.dmmid, fechid.youflixid).exists?
           relation=Dmmlist.where('dmmid = ?', dmm_mv.dmmid).first
           ...
           updaterec = updaterec + 1
            ...
            relation.relationid="dmmtoyouflix"
            relation.frommvid=dmm_mv.dmmid.to_s
            relation.tomvid=fechid.youflixid.to_s
            relation.fromtitle=dmm_mv.dmmtitle.to_s
            relation.totitle=fechid.youflixtitle.to_s
            relation.updatedate=Time.now
            relation.save
          ...
          p 'youflixid:' + fechid.youflixid.to_s
          p 'youflixtitle:' + fechid.youflixtitle.to_s
