...
postlabel = nil
#タグ格納用変数
postgenrestr = "ライブチャット"
#カテゴリ格納変数
postcategorystr = "ライブチャット"
...
#ActiveRecord::Base.logger = Logger.new(STDOUT)
#関連付けリストの投稿日が空白の物を抽出
    relations = Genrerelationlist.where(posteddate: nil).where('genrerelationid = ?', "livechat")
    #relations = Genrerelationlist.where("posteddate = ? and genrerelationid = ?",nil,"livechat")
    #relations = Genrerelationlist.where("genrerelationid = ?","livechat")
    p relations.all.count
    relation  = relations.order("updated_at").limit(30)
...
if relation.count > 0
    relation.each do |data|
        ...
        #メディアが「Sharemovieの場合の投稿情報格納
            ...
            thumbimage =%(http://img1.smv.to//#{tomvdata.smvid}/animation_00001.jpg)
            ...
            p post_title.to_s
            #タグデータ格納
            postgenrestr << "," + tomvdata.tags.to_s
            #カテゴリデータ格納
            postcategorystr << ",sharemovie"
            when ""
        end
        ...
        if user = User2.where("post_name = ?", post_name).exists?
            ...
            genrestrs = postgenrestr.split(",").uniq
