...
#ActiveRecord::Base.logger = Logger.new(STDOUT)
#関連付けリストの投稿日が空白の物を抽出
    relation = Relationlist.order("updated_at").limit(5)
    ...
    relation.each do |data|
        ...
        #SharemovieのID
        p data.tomvid
        smvdata = Smvlist.where('smvid = ?', data.tomvid).first
        #女優名を配列から格納
        ...
        movietag = dmmdata.genre.gsub(",","、") + "、" + smvdata.tags.gsub(",","、")
        ...
        #ビデオ情報
        #動画説明文
        post_content <<  %(<p>#{smvdata.explanation.to_s}</p><br>)
        ...
        post_content <<  "<p>タグ・ジャンル：" +  movietag.gsub("、、","").to_s + "</p><br>"
        ...
        # テーブル名が命名規則に沿わない場合、
        self.table_name = 'wp_2_posts'  # set_table_nameは古いから注意
            self.primary_key = :ID
        end
        ...
        if user = User2.where("post_name = ?", post_name).exists?
            ...
            # テーブル名が命名規則に沿わない場合、
            self.table_name = 'wp_2_postmeta' # set_table_nameは古いから注意
...
=begin            
        ...
        # テーブル名が命名規則に沿わない場合、
        self.table_name = 'wp_2_term_relationships' # set_table_nameは古いから注意
...
=end            
            ...
            # テーブル名が命名規則に沿わない場合、
            self.table_name = 'wp_2_terms' # set_table_nameは古いから注意
            ...
            postgenrestr.split(",").each do |genrestr|
                ...
                # テーブル名が命名規則に沿わない場合、
                self.table_name = 'wp_2_term_taxonomy' # set_table_nameは古いから注意
